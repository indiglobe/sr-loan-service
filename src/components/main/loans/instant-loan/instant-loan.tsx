...
export function InstantLoan() {
  const { banksInstantLoanData } = useLoaderData({
    from: "/loans/instant-loan/",
  });
  ...
  const filteredBanks = banksInstantLoanData.filter(
    (bank) =>
...
export function InstantLoanSeparated() {
  const { banksInstantLoanData } = useLoaderData({
    from: "/loans/instant-loan/",
  });
  const { "bank-name": bankName } = useSearch({
...
export function IndividualInstantLoan() {
  const router = useRouter();
  const { bankDetails: selectedBank } = useLoaderData({
    from: "/loans/instant-loan/$bankId/",
  });
